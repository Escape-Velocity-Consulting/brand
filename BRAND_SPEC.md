...
│   │   ├── ev-wordmark-300.png
│   │   └── logo-dark-300.png
│   └── qr/                        ← generated by generate_qr.py (4 variants × PNG + SVG: cream/transparent × logo/no-logo)
│       ├── qr-hi.{png,svg}
│       ├── qr-hi-plain.{png,svg}
│       ├── qr-hi-transparent.{png,svg}
│       └── qr-hi-transparent-plain.{png,svg}
├── templates/
...
| `assets/qr/qr-hi.{png,svg}` | 860×860 / vector | URL: /hi/ — cream bg + EV logo (canonical) | `generate_qr.py` | Active |
| `assets/qr/qr-hi-plain.{png,svg}` | 860×860 / vector | URL: /hi/ — cream bg, no logo | `generate_qr.py` | Active |
| `assets/qr/qr-hi-transparent.{png,svg}` | 860×860 / vector | URL: /hi/ — transparent bg + EV logo | `generate_qr.py` | Active |
| `assets/qr/qr-hi-transparent-plain.{png,svg}` | 860×860 / vector | URL: /hi/ — transparent bg, no logo | `generate_qr.py` | Active |
| `website/qr.png`, `website/qr-lockscreen.png` | varies | URL: /hi/ — committed copies in the website repo | `generate_qr.py` + manual | Active |

> SVG variants embed the EV monogram from `assets/logos/monogram-dark-square.svg` as vector paths (no font dependency) and are scalable to any size; PNG variants render the logo with Space Grotesk via Pillow.
...
- **`@type: closing` mirrors title chrome.** The last slide of a deck. Same logo + author byline + QR auto-bake as `title`, but for the end-card. Both `title` and `closing` slides receive the QR bake on `publish_artifact`. Supports the same `@bg:`, `@author:`, `@date:`, `@qr:`, `@qr-image:`, `@qr-caption:` directives as `title`.
- **`@type: statement` is a centered impact slide.** Headline rendered at `clamp(52px, 6vw, 96px)`, chrome (logo + page number) visible. `**bold**` in the `# Headline` renders as a terracotta accent word (cream on dark/terracotta bg). An optional second paragraph becomes a smaller subtitle. Use for thesis statements, provocations, and pause moments.
...
- **Author dedup.** When the title or closing slide auto-emits the default `Tommi Enenkel` byline, any subtitle line containing the author tokens is stripped to avoid duplication.
...
|-----------|--------|---------|-------|
| `<!-- @type: X -->` | `title`, `closing`, `section`, `statement`, `content`, `two-col`, `comparison`, `cards`, `big-number`, `quote`, `image`, `html` | `content` | Slide layout |
...
| `<!-- @author: ... -->` | free text | `Tommi Enenkel` | Title / closing-slide author byline override |
| `<!-- @date: ... -->` | free text | — | Title / closing-slide date appended to author byline |
| `<!-- @qr: X -->` | `none`, URL | publish-time auto-bake | Title / closing-slide QR slot — `none` suppresses; explicit URL bakes that destination |
| `<!-- @qr-image: <url> -->` | url | — | Title / closing-slide static image in the QR slot (alternative to QR) |
...
> The relative `./fonts/*` references above are for the **local CLI bundle**, where the viewer sits next to the copied `fonts/` dir. When the **remote MCP server publishes** a deck, the bundle ships no `fonts/` dir, so the served `index.html` is rendered with an **absolute** font URL (`${publicBaseUrl}/fonts`, served by the `/fonts/` route) instead. The two are rendered separately — see the asset-URL invariant in `docs/mcp-server.md`. (`./components/*` still has the relative-path gap when published.)
...
The `escape-velocity-brand` Claude skill is the primary on-brand asset creator. It is a self-sufficient bundle (~340 KB compressed) that ships every source needed to author any branded surface — templates, tokens, fonts, logos, the brand reference, this spec, and a website-prototyping web kit. When asked for a branded asset, the skill reads its own bundled files, substitutes variables, and hands the user finished HTML. The output is openable in any browser with no further infrastructure.

The skill is paired with the `escape-velocity-brand` **MCP server** (see §11.0), which is an **optional extension** adding two capabilities the skill can't provide on its own: Playwright-based pixel rendering (HTML → PNG/PDF) and server-side publishing (promoting renders to permanent URLs on the Brand Site). The split is deliberate:

- **Skill = author + design system.** Knowledge and source materials.
- **MCP = renderer + publisher.** Playwright infrastructure and server-side state.

Anything the skill can produce without Playwright is produced by the skill. The MCP exists for binary output and persistence — nothing the skill can already do.

**Bundle contents** (built by `npm run build:skill` to `dist/escape-velocity-brand.skill`):

- `SKILL.md` — mental model, capability matrix (skill alone vs + MCP), authoring flows for both, routing table, examples
- `references/brand-reference.md` — token table + design patterns (sourced from `BRAND_SKILL.md`)
- `references/brand-spec.md` — this document (canonical contract)
- `references/templates.meta.json` — template registry as JSON (mirrors `templates.meta.ts`)
- `tokens/tokens.css` and `tokens/tokens.json` — design tokens
- `templates/` — all 17 templates including `_base.html` and `_recipient.html`
- `fonts/` — Space Grotesk, Inter, Manrope, JetBrains Mono (woff2 + licenses)
- `assets/logos/` — brand wordmark and logo SVGs
- `components/radar.js` — portable JS component referenced by slide templates
- `web/` — Brand Kit web bundle for prototyping (starter.html, site.css, print.css, README)
- `press/boilerplate.md` — official "about us" copy

**Explicitly excluded:** `assets/raster/` (regenerated by the MCP from SVG sources), `press/photos/` (bulk images), `site/` (the 11ty website source), `scripts/`, `node_modules/`, `dist/`.

**Maintenance rule:** when a new template, token, logo, or web bundle file is added, run `npm run build:skill` to re-stage the bundle and rebuild `dist/escape-velocity-brand.skill`. The auto-generated catalog and routing blocks in `SKILL.md` regenerate from `templates.meta.ts` on every build, and `references/templates.meta.json` is emitted alongside so the bundle's catalog stays in sync with the registry.
