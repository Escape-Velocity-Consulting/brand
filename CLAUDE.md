...
|--------|---------|--------------|
| `dev` | `build:tokens && build:logos && build:site && eleventy --serve` | Regenerate tokens + logos, render Brand Site, then serve at `localhost:3000/brand/` with live reload. **Primary iteration command.** Skips slow asset/kit builds. |
| `build:tokens` | `tsx scripts/build-tokens.ts` | Regenerate `tokens.css` + `tokens.json` from `tokens.ts` |
| `build:logos` | `tsx scripts/generate-logos.ts` | Regenerate the 54-variant logo set (SVGs + `logos.manifest.json`) from `logos.config.ts` + `_glyphs.json`. Pure Node. Run before `build:assets`. |
| `build:assets` | `tsx scripts/export-assets.ts` | Regenerate all rasters + document previews. Slow (Playwright). Reads `logos.manifest.json`. |
...
├── logos.config.ts           ← logo variant matrix (mark × frame × surface) + colour mapping; source for build:logos
...
├── SpaceGrotesk.ttf          ← variable font; source for QR + the 700 instance
├── fonts/                    ← woff2 fonts (canonical home)
│   ├── SpaceGrotesk-700.ttf  ← static 700 instance (one-time, for glyph extraction)
...
│   └── photos/
├── assets/
│   ├── logos/                ← generated logo SVGs + aliases + _glyphs.json (build:logos)
│   ├── logos.manifest.json   ← generated logo variant manifest (drives raster + Brand Site)
